		...
		controller : function($scope, $element, $attrs) {
		   ...
		   $scope.imageArea = function(parent, areaid) {
		        var options = parent.options,
		            ...
		            updateBtLink = function (visible) {
		                if ($btLink) {
		                	$btLink.css({
		                        display: visible ? "block" : "none",
		                        left: area.x + area.width + 1,
		                        top: area.y + area.height - 20,
		                        "z-index": area.z + 1
		                    });
		                }
		            },
		            ...
		            refresh = function(sender) {
		                switch (sender) {
		                    case "startSelection":
		                    	if(selectionPoint[0] == area.x && selectionPoint[1] == area.y) {
		                    		return;
		                    	}
		                        parent._refresh();
		                        updateSelection();
		            ...
		            startSelection  = function (event) {
		                ...
		                selectionPoint[0]=area.x;
		                selectionPoint[1]=area.y;
		                refresh("startSelection");
		            },
		            pickSelection = function (event) {
		                cancelEvent(event);
		                focus();
		                console.log("pick area");
		            ...
		            getAreaCenterPoint = function(event) {
		            	var width = area.width;
		            	var height = area.height;

		            	var centerX = area.x + width / 2;
		            	var centerY = area.y + height / 2;
		        ...
		        $selection = $("<img src=\"resource/image/elements/"+$scope.objectImg.img+"\" class=\"ngAreas-element  border-medium\" />")
		        ...
		        var bindToDrag = function($obj) {
		        	return $obj;
		        };
		        $btLink = bindToDrag($("<div class=\"ngAreas-element link-area\" />"))
		        .bind('dragstart', function(event) { event.preventDefault(); })
                .append(bindToDrag($("<div class=\"ngAreas-element select-areas-link-area\" />")))
                .insertAfter($selection);
